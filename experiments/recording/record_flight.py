...
DATA_DIR = Path(__file__).parent.parent / "data" / "drone-exp"
...
def main(setting: str, controller: int, rosbag_path: Path, speed: float) -> None:
    ...
    match setting:
        case "promis":
            path = load(DATA_DIR / f"original_path-speed-{speed:.2f}.npy")
        case "coco-fixed-v":
            path = load(DATA_DIR / f"augmented_path-controller-{controller}-speed-{speed:.2f}.npy")
        case "coco-dynamic-v":
            path = load(DATA_DIR / f"extended_path-controller-{controller}.npy")
